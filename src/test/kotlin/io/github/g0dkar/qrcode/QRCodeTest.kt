...
internal class QRCodeTest {
    ...
    fun `render - numbers - as numbers`() {
        ...
        val resultImage = (result as BufferedImageCanvas).image
    ...
    fun `render - performance`() {
        ...
        val javaTime = chronometer {
            val javaQRCode = generateJavaQRCode(input, mode = Mode.MODE_8BIT_BYTE)
            javaQRCode.createImage(25, 0, Colors.WHITE, Colors.RED)
        }

        val kotlinTime = chronometer {
            val underTest = QRCode(input)
            underTest.render(darkColor = Colors.RED)
        }

        kotlinTime shouldBeLessThanOrEqualTo javaTime
    }
    ...
    private fun chronometer(action: () -> Unit): Instant =
        Instant.now().let {
            action()
            return Instant.now().minusMillis(it.toEpochMilli()).minusNanos(it.nano.toLong())
        }
}
